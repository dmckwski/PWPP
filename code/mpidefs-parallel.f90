      ...
      contains
         ...
         if(present(mpi_recv_buf_char).or.present(mpi_send_buf_char)) then
            ...
            if(mpi_command.eq.'bcast') then
               call MPI_BCAST (mpi_send_buf_char,mpi_number,type,mpi_rank,comm,ierr)
               return
            endif
         endif
         ...
         if(present(mpi_recv_buf_i).or.present(mpi_send_buf_i)) then
            ...
            if(mpi_command.eq.'send') then
               call mpi_send(mpi_send_buf_i,mpi_number,type,mpi_rank,mpitag,comm,ierr)
               return
            endif
            if(mpi_command.eq.'recv') then
               call mpi_recv(mpi_recv_buf_i,mpi_number,type,mpi_rank,mpitag,comm,stat,ierr)
               return
            endif
         ...
         if(present(mpi_recv_buf_r).or.present(mpi_send_buf_r)) then
            ...
            if(mpi_command.eq.'send') then
               call mpi_send(mpi_send_buf_r,mpi_number,type,mpi_rank,mpitag,comm,ierr)
               return
            endif
            if(mpi_command.eq.'recv') then
               call mpi_recv(mpi_recv_buf_r,mpi_number,type,mpi_rank,mpitag,comm,stat,ierr)
               return
            endif
            if(mpi_command.eq.'reduce') then
         ...
         if(present(mpi_recv_buf_c).or.present(mpi_send_buf_c)) then
            ...
            if(mpi_command.eq.'send') then
               call mpi_send(mpi_send_buf_c,mpi_number,type,mpi_rank,mpitag,comm,ierr)
               return
            endif
            if(mpi_command.eq.'recv') then
               call mpi_recv(mpi_recv_buf_c,mpi_number,type,mpi_rank,mpitag,comm,stat,ierr)
               return
            endif
            if(mpi_command.eq.'reduce') then
         ...
         if(present(mpi_recv_buf_dp).or.present(mpi_send_buf_dp)) then
            ...
            if(mpi_command.eq.'send') then
               call mpi_send(mpi_send_buf_dp,mpi_number,type,mpi_rank,mpitag,comm,ierr)
               return
            endif
            if(mpi_command.eq.'recv') then
               call mpi_recv(mpi_recv_buf_dp,mpi_number,type,mpi_rank,mpitag,comm,stat,ierr)
               return
            endif
            if(mpi_command.eq.'reduce') then
               if(present(mpi_send_buf_dp)) then
                  ...
                  call mpi_reduce(dptemp,mpi_recv_buf_dp,mpi_number,type,mpi_operation, &
                               mpi_rank,comm,ierr)
                  deallocate(dptemp)
               endif
               return
         ...
         if(present(mpi_recv_buf_dc).or.present(mpi_send_buf_dc)) then
            ...
            if(mpi_command.eq.'send') then
               call mpi_send(mpi_send_buf_dc,mpi_number,type,mpi_rank,mpitag,comm,ierr)
               return
            endif
            if(mpi_command.eq.'recv') then
               call mpi_recv(mpi_recv_buf_dc,mpi_number,type,mpi_rank,mpitag,comm,stat,ierr)
               return
            endif
            if(mpi_command.eq.'reduce') then
               if(present(mpi_send_buf_dc)) then
                  ...
                  call mpi_reduce(dctemp,mpi_recv_buf_dc,mpi_number,type,mpi_operation, &
                               mpi_rank,comm,ierr)
                  deallocate(dctemp)
               endif
               return
            ...
            if(mpi_command.eq.'allreduce') then
               if(present(mpi_send_buf_dc).and.present(mpi_recv_buf_dc)) then
                  ...
                  allocate(dctemp(mpi_number))
                  dctemp(1:mpi_number)=mpi_recv_buf_dc(1:mpi_number)
                  call mpi_allreduce(dctemp,mpi_recv_buf_dc,mpi_number,type,mpi_operation, &
                               mpi_rank,comm,ierr)
                  deallocate(dctemp)
               endif
               return
            ...
            if(mpi_command.eq.'allgather') then
               if(present(mpi_send_buf_dc).and.present(mpi_recv_buf_dc)) then
                  call mpi_allgather(mpi_send_buf_dc,mpi_number,type,mpi_recv_buf_dc,mpi_number,type, &
                               comm,ierr)
               else
                  call mpi_gather(mpi_in_place,mpi_number,type,mpi_recv_buf_dc,mpi_number,type, &
                               comm,ierr)
               endif
               return
            endif
            if(mpi_command.eq.'alltoall') then
               if(present(mpi_send_buf_dc).and.present(mpi_recv_buf_dc)) then
                  call mpi_alltoall(mpi_send_buf_dc,mpi_number,type,mpi_recv_buf_dc,mpi_number,type, &
                               comm,ierr)
               else
                  call mpi_alltoall(mpi_in_place,mpi_number,type,mpi_recv_buf_dc,mpi_number,type, &
                               comm,ierr)
               endif
               return
            endif
            if(mpi_command.eq.'scatter') then
               if(present(mpi_send_buf_dc).and.present(mpi_recv_buf_dc)) then
                  call mpi_scatter(mpi_send_buf_dc,mpi_number,type,mpi_recv_buf_dc,mpi_number,type, &
                               mpi_rank,comm,ierr)
               else
                  call mpi_scatter(mpi_recv_buf_dc,mpi_number,type,mpi_recv_buf_dc,mpi_number,type, &
                               mpi_rank,comm,ierr)
               endif
               return
            endif
